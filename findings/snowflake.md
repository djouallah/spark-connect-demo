...
| **No thin Spark Connect client from a laptop** | Spark Connect exists so that a client needs only `pyspark-client`: no JVM and no full PySpark (that's how `local.ipynb` works). Snowflake has no hosted `sc://` endpoint. Its only supported laptop client, `snowpark-connect` (1.44.0), runs the Spark Connect server locally: it vendors full PySpark 3.5.6 plus Spark jars and has hard dependencies on `jpype1` and `py4j`, so it needs a JVM (the `[jdk]` extra ships a JRE). That defeats the point of Spark Connect: without Java on the laptop, there is no interactive Spark on Snowflake. | [Develop with a local IDE](https://docs.snowflake.com/en/developer-guide/snowpark-connect/snowpark-connect-local-ide) | None from a laptop: submit a Code Bundle (`platforms/snowflake/run.py`) |

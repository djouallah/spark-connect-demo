...
JOBS = {
    "simple":       dict(job="jobs/simple.py", spec="simple", result="SIMPLE_DEMO"),
    "etl":          dict(job="jobs/etl.py", spec="etl", data="data/etl", stage="CODE_BUNDLE_TEST",
                         args=["--raw", "@TPCH.PUBLIC.CODE_BUNDLE_TEST/raw", "--schema", "TPCH.PUBLIC"],
                         result="ETL_AGG_DAILY_COUNTRY"),
    "coffee_gen":   dict(job="jobs/coffee_gen.py", spec="coffee", data="data/coffee", stage="COFFEE_RAW",
                         args=["--raw", "@TPCH.PUBLIC.COFFEE_RAW/raw/coffee_dims", "--schema", "TPCH.COFFEE"]),
    "coffee_bench": dict(job="jobs/coffee_bench.py", spec="coffee",
                         args=["--sql", "@TPCH.PUBLIC.COFFEE_RAW/raw/coffee_sql/queries.sql", "--schema", "TPCH.COFFEE"],
                         result="TPCH.COFFEE.TIMINGS_SPARK"),
    "dml":          dict(job="jobs/dml.py", spec="dml", args=["--schema", "TPCH.DML", "--tag", "ext"],
                         result="TPCH.DML.RESULTS_EXT"),
    "dml_noext":    dict(job="jobs/dml.py", spec="dml_noext", args=["--schema", "TPCH.DML", "--tag", "noext"],
    ...
    "tpch":         dict(job="jobs/tpch.py", spec="tpch", data="data/tpch", stage="TPCH_RAW",
